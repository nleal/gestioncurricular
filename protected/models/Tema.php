...
class Tema extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('status, titulo, id_materia', 'required'),
			array('id_materia', 'numerical', 'integerOnly'=>true),
			array('descripcion, titulo', 'length', 'max'=>50),
			array('status', 'length', 'max'=>2),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('descripcion, status, titulo, id_tema, id_materia', 'safe', 'on'=>'search'),
		);
	}
